#!/usr/bin/env python3
"""fetch_marine_warnings.py -- OHB backend feeder for HamClock's Marine Wx overlay.

Polls api.weather.gov for active US NWS marine products and writes:
  1. a small flat summary file at the path given on the command line (served as
     /marine/warnings.txt), which marinewarnings.cpp fetches and parses every ~5 min.
  2. one plain-text file per active warning in a "detail/" directory alongside it (served as
     /marine/detail/<sanitized-id>.txt), containing the full NWS description/instruction text.
...
where AREA is the CAP alert's areaDesc (human-readable affected zone, e.g. "Coastal waters
from Cape Henlopen to Fenwick Island DE out 20 nm") and VERTS (if present) is
"lat1:lon1;lat2:lon2;...;latN:lonN". VERTS is always the last field so it may itself contain
further commas -- it doesn't, by construction below, but the client parser is written to
tolerate that regardless.

Usage:
    fetch_marine_warnings.py
(writes to /opt/hamclock-backend/htdocs/ham/HamClock/marine/warnings.txt -- see HAMCLOCK_BASE
...
NWS_URL = ("https://api.weather.gov/alerts/active"
           "?event=Special%20Marine%20Warning,Marine%20Weather%20Statement,Storm%20Warning")
USER_AGENT = "HamClock-OHB/1.0 (fetch_marine_warnings.py)"
TIMEOUT_S = 15
...
def extract_polygon(geometry):
    ...
    if len(verts) > 1 and verts[0] == verts[-1]:
        verts = verts[:-1]
    return verts[:20]                # cap matches MARINE_MAXVERTS in marinewarnings.cpp


def sanitize_id_for_path(alert_id):
    """MUST exactly match sanitizeIdForPath() in marinewarnings.cpp: isalnum, '.', '_', '-'
...
def build_lines(geojson):
    ...
    for feature in geojson.get("features", []):
        ...
        if verts:
            ...
            # no polygon -- fall back to the affected-zone UGC centroid isn't available from
            # this endpoint without a second lookup, so skip warnings with no geometry at all
            # rather than guess; the vast majority of SMW/marine products do include one.
            continue
...
OUT_PATH = os.path.join(HAMCLOCK_BASE, "marine", "warnings.txt")
...
def main():
    ...
    try:
        ...
        sys.stderr.write(f"fetch_marine_warnings: fetch/parse failed: {e}\n")
    ...
    with open(tmp_path, "w") as f:
        f.write("# generated by fetch_marine_warnings.py -- do not edit\n")
        for line in lines:
            f.write(line + "\n")
    os.replace(tmp_path, out_path)     # atomic swap so the client never sees a half-written file

    # detail/ lives alongside warnings.txt, served as /marine/detail/<id>.txt
    ...
    for safe_id, text in details.items():
        ...
        det_tmp = det_path + ".tmp"
        with open(det_tmp, "w") as f:
    ...
    print(f"fetch_marine_warnings: wrote {len(lines)} warnings and "
